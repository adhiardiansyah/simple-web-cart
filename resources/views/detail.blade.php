...
@section('title', 'Toko Adhi | '.$product->name)
@section('content')
      ...
      <div class="col-md-3">
        <div class="card p-5 shadow-sm border-0">
          ...
          <div class="d-flex flex-column">
            <!-- Beli -->
            <form action="/buy" class="d-flex mb-2" method="POST"> @csrf
              <input type="hidden" name="product_id" value="{{ $product->id }}">
              <input type="hidden" name="price" value="{{ $product->price }}">
              <input type="number" name="qty" class="form-control me-2 shadow-none" min=" 1" value="1" required>
            ...
            <form action="/addToCart2" method="POST"> @csrf
              <input type="hidden" name="product_id" value="{{ $product->id }}">
              <input type="hidden" name="price" value="{{ $product->price }}">
              <button type="submit" class="shadow-none btn btn-success w-100" title="Tambah ke keranjang!">
