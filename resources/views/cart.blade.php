...
@section('title', 'Toko Adhi | Keranjang')
@section('content')
  ...
  <div class="container">
      ...
      <div class="">
        <table class="table">
          ...
          <tbody>
            @foreach ($cart as $c)
            <tr>
              <td class="col-6 py-5 fw-bold" style="vertical-align:middle">{{ $c->product->name }}</td>
              <td class="col-3 py-5" style="vertical-align:middle">Rp.
                {{ number_format($c->price,0,',','.') }}</td>
              <form action="/update-cart" method="post"> @csrf
                <td class="col-2 py-5" style="vertical-align:middle">
                  <input type="number" name="qty" value="{{ $c->qty }}"
                    class="form-control bg-transparent border-0 shadow-none fw-bold" min="1" required>
                </td>
                <td class="col-1 py-5">
              ...
              <a href="#" class="btn bg-transparent shadow-none deleteCart" data-id="{{ $c->id }}" title="Hapus">
    ...
    <div class="col-10 mt-5">
      ...
      <table class="table">
        ...
        <tr>
          <td class="py-3">Total kupon yang didapatkan:</td>
          <td class="py-3 text-end fw-bold">
            {{ $totalcoupon }}
          </td>
        </tr>
          ...
          <td class="py-3 text-end fw-bold">
            <form action="/checkout" method="post" class="d-flex"> @csrf
              <textarea name="destination" class="form-control bg-light" cols="30" rows="2"
                placeholder="Jalan, Dusun, Desa/Kelurahan, Kecamatan, Kabupaten/Kota"></textarea>
          </td>
        </tr>
      </table>
    </div>
